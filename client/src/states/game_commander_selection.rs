...
const STATE: AppState = AppState::GAME_COMMANDER_SELECTION;
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let frame = asset_server.load("vox/commander_frame.vox");
    // add entities to the world
    commands
        .spawn_bundle((
            Transform::from_xyz(-65.0, -20.0, 0.0),
            GlobalTransform::identity(),
        ))
        .with_children(|parent| {
            parent.spawn_scene(frame.clone());
        })
        .insert(Cleanup);
    commands
        .spawn_bundle((
    ...
    commands
        .spawn_bundle((
            Transform::from_xyz(25.0, -20.0, 0.0),
            GlobalTransform::identity(),
        ))
        .with_children(|parent| {
            parent.spawn_scene(frame);
        })
        .insert(Cleanup);
}
...
fn button_click(
    ...
    for ev in ev_button_click.iter() {
        match ev.0.as_str() {
            "btn_select" => ev_state_change.send(StateChangeEvent(AppState::MENU_MAIN)),
            _ => (),
        }
    }
}
...
fn timer(
    ...
    for (watch, mut text) in timer.iter_mut() {
        ...
        } else if watch.time.percent() >= 0.5 {
