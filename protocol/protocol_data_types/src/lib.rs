use protocol_types::{character::CharacterUpgrade, heros::Pantheon};
...
impl ToString for CharacterJson {
    fn to_string(&self, idx: usize) -> String {
        format!(
            "Character {{ id: {}, name: Borrowed(\"{}\"), description: Borrowed(\"{}\"), health: {}, attack: {}, cost: {}, upgrade: {}}}",
            idx,
            self.name,
            self.description,
            self.health,
            self.attack,
            self.cost,
            if let Some(upgrade) = &self.upgrade { format!("Some(CharacterUpgrade {{ name: Borrowed(\"{}\"), attack: {}, health: {} }})", upgrade.name, upgrade.attack, upgrade.health) } else { "None".to_string() }
        )
    }
}
